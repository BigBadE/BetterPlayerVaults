...
public class ClearCommand extends BasicCommand<CommandSender> {
  private static final String VAULT_CLEARED_MESSAGE =
      new StringMessage("command.clear.success").translate();
  public static final String COMMAND_USAGE_MESSAGE =
      new StringMessage("command.clear.usage").translate();
  ...
  public void onCommand(CommandSender player, String[] args) {
    if (args.length == 1) {
      ...
      vaultOptional.ifPresent(
          vault
          -> getPlayer(player, args[0],
                       target
                       -> {
                       ...
                       target -> {
                         getVaultManager().resetVaultOffline(
                             target.getUniqueId(), vault);
                         player.sendMessage(VAULT_CLEARED_MESSAGE);
                       }));
    } else {
