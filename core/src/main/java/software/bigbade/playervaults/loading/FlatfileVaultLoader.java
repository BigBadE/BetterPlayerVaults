...
public class FlatfileVaultLoader implements IVaultLoader {
    ...
    public CompletableFuture<Inventory> getVault(UUID player, int vault, int size) {
        ...
        new ActionChain(player.toString()).async(() -> {
            ...
            if (!dataFile.exists()) {
                future.complete(Bukkit.createInventory(null, size, ExternalDataLoader.VAULT_TITLE.translate(vault)));
            }
            future.complete(SerializationUtils.deserialize(FileUtils.read(dataFile), ExternalDataLoader.VAULT_TITLE.translate(vault), size));
        }).execute();
