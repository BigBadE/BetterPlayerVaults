...
public abstract class ExternalDataLoader implements IVaultLoader {
  ...
  @Getter private static final Logger logger = PlayerVaults.getPluginLogger();
        ...
        .async(() -> {
          ...
          if (data.length == 0) {
            future.complete(Bukkit.createInventory(
                null, size, VAULT_TITLE.translate(vault)));
          } else {
            future.complete(SerializationUtils.deserialize(
                data, VAULT_TITLE.translate(vault), size));
          }
        })
        .execute();
    return future;
  ...
  public void saveVault(UUID player, IPlayerVault vault) {
    saveData(player, vault.getNumber(),
