...
public abstract class PlayerVaults extends JavaPlugin {
  private static final URL VERSION_URL = FileUtils.getURL(
      "https://gitcdn.link/repo/BigBadE/BetterPlayerVaults/release/version.yml");
  ...
  public void initVersion() {
    ...
    if (!versionFile.exists()) {
      ...
      setVersion(
          new String(FileUtils.read(versionFile), StandardCharsets.UTF_8));
    }
    ...
    try (InputStream stream = FileUtils.createStream(VERSION_URL)) {
      ...
      if (stream.read(buffer) == 0) {
        getLogger().log(Level.SEVERE, "Version file ({0}) had 0 bytes",
                        VERSION_URL);
      }
