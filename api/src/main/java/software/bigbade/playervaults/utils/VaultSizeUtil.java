...
public final class VaultSizeUtil {
  ...
  public static int getSize(Player player) {
    int size = VaultSizeUtil.getPermissionInteger(
        player, "betterplayervaults.vaults.size.", 27);
  ...
  public static int getVaults(Player player) {
    return VaultSizeUtil.getPermissionInteger(
        player, "betterplayervaults.vaults.amount.", 1);
  }

  private static int getPermissionInteger(Player player, String permissionStr,
                                          int defaultValue) {
    Optional<String> permission =
        player.getEffectivePermissions()
            ...
            .findAny();
    try {
      return permission.map(string -> string.substring(permissionStr.length()))
          .map(Integer::parseInt)
          .orElse(defaultValue);
    } catch (NumberFormatException e) {
      PlayerVaults.getPluginLogger().log(
          Level.SEVERE, "Player {0} has invalid permission {1}",
          new Object[] {player.getName(), permission.get()});
      return defaultValue;
    }
  }
}
